"""DFlash2 draft model for vLLM: the grouped convolution and the selector.

A port of ``vllm/model_executor/models/qwen3_dflash2.py`` from
vllm-project/vllm#52816, carried here so a checkpoint trained by this repo can be
served by a vLLM that predates the PR. ``_grouped_conv`` and ``_score_edges`` are
the PR's implementations verbatim -- they are also
:func:`speculators.models.dflash2.model_definitions.grouped_conv` and
:func:`~speculators.models.dflash2.model_definitions.score_edges` verbatim, which
is what makes a checkpoint draft with the arithmetic it was trained with.

One thing the PR gets from vLLM and this module cannot: #52816 adds
``decoder_layer_cls`` to ``DFlashQwen3Model`` and ``model_cls`` to
``DFlashQwen3ForCausalLM`` so the DFlash2 classes can subclass them. Before the
PR both are hardcoded lookups in the base ``__init__``, so :func:`_substituted`
rebinds the module global for the duration of the constructor. Model construction
is single-threaded, which is what makes that safe; when the hooks are present
...
def _flashinfer_topk() -> Callable[..., tuple[torch.Tensor, torch.Tensor]] | None:
    ...
    if not has_flashinfer():
        ...
        return None
    from flashinfer import top_k  # noqa: PLC0415
...
@contextmanager
def _substituted(name: str, replacement: type) -> Iterator[None]:
    """Rebind one ``qwen3_dflash`` global while a base constructor runs."""
    original = getattr(qwen3_dflash, name)
    setattr(qwen3_dflash, name, replacement)
    try:
        yield
    finally:
        setattr(qwen3_dflash, name, original)
...
class DFlash2Qwen3DecoderLayer(DFlashQwen3DecoderLayer):
    def __init__(
        ...
        *,
        ...
        assert speculative_config is not None  # noqa: S101
        conv_args = {
    ...
    def __init__(
        ...
        *,
        ...
        with ExitStack() as stack:
            if not hasattr(DFlashQwen3Model, "decoder_layer_cls"):
                stack.enter_context(
                    _substituted("DFlashQwen3DecoderLayer", DFlash2Qwen3DecoderLayer)
                )
            super().__init__(
