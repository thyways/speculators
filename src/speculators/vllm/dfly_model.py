...
"""vLLM model implementation for Speculators DFly checkpoints.
...
class HiddenStatesCorrection(nn.Module):
    """Previous-token-conditioned SwiGLU residual used by DFly."""
    ...
    def forward(
        ...
        correction_input = torch.cat(
            [hidden_norm, embed_norm],
            dim=-1,
        )
        gate_up, _ = self.gate_up_proj(correction_input)
        correction, _ = self.down_proj(self.act_fn(gate_up))
...
class Qwen3DFlyModel(DFlashQwen3Model):
    ...
    def __init__(
        ...
        *,
        ...
        if bool(getattr(config, "enable_hidden_correction", True)):
            correction_type = str(
    ...
    def _build_fused_kv_buffers(self) -> None:
        ...
        self._fusion_probs.copy_(
            functional.softmax(self.layer_fusion_weights.float(), dim=-1).to(
...
class Qwen3DFlyForCausalLM(Qwen3DSparkForCausalLM):
    ...
    def has_hidden_correction(self) -> bool:
        return self.model.hidden_correction is not None
    ...
    def apply_hidden_correction(
        ...
        if self.model.hidden_correction is None:
    ...
    def load_weights(  # noqa: C901
        ...
        for weight_name, loaded_weight in weights:
            name = weight_name
            if any(part in name for part in _TRAINING_ONLY_WEIGHTS):
                continue
            if "d2t" in name:
        ...
        for checkpoint_name, loaded_weight in model_weights.items():
