...
class GenerationArgs(_Group):
    ...
    fail_on_hidden_state_error: bool = Field(
        default=False,
        description=(
            "Abort immediately when online hidden-state generation fails or returns "
            "token IDs that differ from the prepared sample, instead of retrying and "
            "dropping the sample."
        ),
    )
    ...
    def _validate_dpace(self) -> "TrainConfig":
        ...
        if self.dflash.per_position_loss_weight == "dpace":
            if self.loss.loss_fn != "ce":
                raise ValueError("D-PACE per-position weighting requires --loss-fn=ce")
