...
from speculators.convert.domino.converter import DominoConverter
...
def convert_model(
    model: str,
    verifier: str,
    algorithm: Literal["eagle3", "mtp", "dflash", "domino"],
    ...
    **kwargs,
    ...
    algorithm=="domino":
        Domino: https://github.com/sgl-project/SpecForge
        ::
        convert_model(
            model="./domino/checkpoint",
            verifier="Qwen/Qwen3-8B",
            algorithm="domino",
        )
    ...
    :param kwargs: Additional keyword arguments for the conversion algorithm.
        ...
        Options for Domino: {"aux_hidden_state_layer_ids": [2,10,18,26,34]};
        the recurrent head's dimensions are read from the checkpoint's
        dflash_config. Requires draft_vocab_size == vocab_size.
    """
...
def maybe_convert_external_checkpoint(
    ...
    dflash_config = config_dict.get("dflash_config") or {}
    if dflash_config.get("projector_type") == "domino" or any(
        "Domino" in architecture for architecture in architectures
    ):
        algorithm: Literal["dflash", "domino"] = "domino"
    elif "dflash_config" in config_dict or any(
        "DFlash" in architecture for architecture in architectures
    ):
        algorithm = "dflash"
    else:
        raise NotImplementedError(
            f"Cannot auto-convert checkpoint '{model}': unrecognized external "
            "format. Supported auto-conversion: DFlash and Domino."
        )
