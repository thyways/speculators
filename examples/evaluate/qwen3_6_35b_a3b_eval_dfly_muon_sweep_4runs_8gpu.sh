#!/usr/bin/env bash
# Add the Muon-trained DFlash checkpoint and the DFly checkpoint to an existing
# draft sweep produced by qwen3_6_35b_a3b_eval_draft_sweep_5runs_8gpu.sh.
#
# Four runs, executed one after another (each run occupies all eight GPUs as
# eight independent single-GPU replicas, per-replica concurrency 1, greedy):
#
#   1. dflash_muon, 7 speculative tokens
#   2. dflash_muon, 15 speculative tokens
#   3. dfly, 7 speculative tokens
#   4. dfly, 15 speculative tokens
#
# Like the Domino follow-up this script does not create a new sweep root: it
# appends `<label>` directories and `sweep_summary.txt` rows to SWEEP_ROOT,
# which must already exist. Settings are kept identical to the five-run sweep so
# the resulting columns are directly comparable.
#
# Neither checkpoint's path follows the `<algorithm>_qwen3_6_35b_a3b_5full`
# convention the five-run sweep derives paths from (the Muon run is a second
# DFlash variant, and DFly lives outside the shared draft root), so each run
# carries its draft path and its runner algorithm explicitly.
#
...
# label:runner_algorithm:speculative_tokens:draft_model_path
RUNS=(
    "dflash_muon_spec7:dflash:7:$WORKSPACE/model_weights/dflash_qwen3_6_35b_a3b_5full_muon/checkpoints/0"
    "dflash_muon_spec15:dflash:15:$WORKSPACE/model_weights/dflash_qwen3_6_35b_a3b_5full_muon/checkpoints/0"
    "dfly_spec7:dfly:7:$WORKSPACE/model_weights/dfly_qwen3_6_35b_a3b_5full/checkpoints/0"
    "dfly_spec15:dfly:15:$WORKSPACE/model_weights/dfly_qwen3_6_35b_a3b_5full/checkpoints/0"
)
...
for entry in "${RUNS[@]}"; do
    IFS=':' read -r label algorithm spec_tokens model <<< "$entry"
    runner="$SCRIPT_DIR/example_qwen3_6_35b_a3b_${algorithm}_speculator_benchmarks_8gpu.sh"
    if [[ ! -f "$runner" ]]; then
        echo "Missing runner script for $label: $runner" >&2
        exit 1
    fi
    for path in "$model/config.json" "$model/model.safetensors"; do
        if [[ ! -f "$path" ]]; then
...
for entry in "${RUNS[@]}"; do
    ...
    log "=== Run $label: $algorithm runtime, $spec_tokens speculative tokens ==="
    log "    model=$model"
    log "    log=$run_log"
    start_epoch="$SECONDS"
    if env \
    ...
    then
        ...
        printf 'FAIL  %-18s %-7s spec=%-3s exit=%s  %s\n' \
...
log "All four runs complete: $SWEEP_ROOT"
