#!/usr/bin/env bash
# Evaluate the local five-layer Qwen3.6-35B-A3B token-latent-feedback checkpoint
# with RedHatAI/speculator_benchmarks. Throughput mode records speculative
# acceptance metrics together with latency, TTFT, ITL, and throughput metrics.
#
# Smoke test:
#
#   SUBSETS=HumanEval MAX_REQUESTS=4 \
#     bash examples/evaluate/example_qwen3_6_35b_a3b_token_latent_feedback_speculator_benchmarks.sh
#
# Set MODE=sweep for output-length estimation and a multi-rate sweep.
#
# Token-latent feedback has no dedicated vLLM `--spec-method`: it keeps the
# DFlash parallel backbone and adds a block-parallel latent feedback stage, so
# `speculators.vllm.token_latent_feedback` routes it onto the DFlash runtime via
# `register_speculative_method_alias("token_latent_feedback", "dflash")`. That
# rewrite only fires when the *served* model is the speculator (it hangs off
# SpeculatorsConfig.build_vllm_speculative_config); here the verifier is served
# and the draft is passed via --spec-model, so the runtime is spelled out as
# `--spec-method dflash` instead. The plugin still supplies the config
# translation and the Qwen3TokenLatentFeedbackModel registration, both keyed off
# `speculators_model_type=token_latent_feedback`.
# See src/speculators/vllm/token_latent_feedback.py.
...
OUTPUT_DIR="${OUTPUT_DIR:-$WORKSPACE/evaluation_results/token_latent_feedback_qwen3_6_35b_a3b_5swa_ckpt0_spec${NUM_SPECULATIVE_TOKENS}_${TIMESTAMP}}"
...
case "$PREFIX_CACHING" in
    ...
    disabled)
        ...
        ;;
    *)
        echo "PREFIX_CACHING must be default, enabled, or disabled, got: $PREFIX_CACHING" >&2
        exit 1
        ;;
esac
...
# The installed `speculators` dist-info predates this entry point, and a stale
# `speculators.egg-info` under `$REPO/src` can additionally hide site-packages
# metadata once that directory leads PYTHONPATH, so importlib.metadata may not
# see the token-latent-feedback plugin at all. Probe with the PYTHONPATH the
# server will actually use, and add a tiny metadata shim under a different
# distribution name when the plugin is invisible. The environment itself is left
# untouched.
...
raise SystemExit(
    not any(item.name == "speculators_token_latent_feedback" for item in plugins)
)
PY
then
    mkdir -p "$REPO/tmp"
    PLUGIN_METADATA_ROOT="$(mktemp -d "$REPO/tmp/speculators-token-latent-feedback-plugin.XXXXXX")"
    DIST_INFO="$PLUGIN_METADATA_ROOT/speculators_token_latent_feedback_runtime-0.0.0.dist-info"
    mkdir -p "$DIST_INFO"
    printf '%s\n' \
        'Metadata-Version: 2.1' \
        'Name: speculators-token-latent-feedback-runtime' \
        'Version: 0.0.0' > "$DIST_INFO/METADATA"
    printf '%s\n' \
        '[vllm.general_plugins]' \
        'speculators_token_latent_feedback = speculators.vllm.token_latent_feedback:register' \
        > "$DIST_INFO/entry_points.txt"
    PLUGIN_PYTHONPATH="$PLUGIN_PYTHONPATH:$PLUGIN_METADATA_ROOT"
fi

echo "=== Token-latent feedback evaluation configuration ==="
...
echo "Plugin metadata shim:  ${PLUGIN_METADATA_ROOT:-not needed}"
echo "Output:                $OUTPUT_DIR"

echo "=== Step 1: Launching vLLM token-latent feedback server ==="
setsid env \
    CUDA_VISIBLE_DEVICES="$CUDA_VISIBLE_DEVICES" \
    HF_ENDPOINT="$HF_ENDPOINT" \
    PYTHONPATH="$PLUGIN_PYTHONPATH${PYTHONPATH:+:$PYTHONPATH}" \
    VLLM_PLUGINS=speculators_token_latent_feedback \
