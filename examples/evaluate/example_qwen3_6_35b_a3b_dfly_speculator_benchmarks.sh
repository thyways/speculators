#!/usr/bin/env bash
# Evaluate the local five-layer Qwen3.6-35B-A3B DFly checkpoint with
...
#     bash examples/evaluate/example_qwen3_6_35b_a3b_dfly_speculator_benchmarks.sh
#
# Set MODE=sweep for output-length estimation and a multi-rate sweep.
#
# DFly has no dedicated vLLM `--spec-method`: it keeps DFlash's parallel
# backbone but samples the block left to right, which is exactly the DSpark
# runtime's layout, so `speculators.vllm.dfly` routes it there. The
# `register_speculative_method_alias("dfly", "dspark")` rewrite only fires when
# the *served* model is the speculator (it hangs off
...
# `--spec-method dspark` instead. The plugin still supplies the DFly config
# translation, the widened auxiliary-hidden-state buffer, and the
# Qwen3DFlyModel registration, all keyed off `speculators_model_type=dfly`.
# See src/speculators/vllm/dfly.py.
...
OUTPUT_DIR="${OUTPUT_DIR:-$WORKSPACE/evaluation_results/dfly_qwen3_6_35b_a3b_5full_ckpt0_spec${NUM_SPECULATIVE_TOKENS}_${TIMESTAMP}}"
...
# per name, so putting that directory ahead of site-packages on PYTHONPATH hides
# the installed dist-info together with every `vllm.general_plugins` entry
# point. Probe with the PYTHONPATH the server will actually use, and add a tiny
# metadata shim under a different distribution name when the DFly plugin is
# invisible. The environment itself is left untouched.
if ! env PYTHONPATH="$PLUGIN_PYTHONPATH" "$VLLM_PYTHON" - <<'PY'
from importlib.metadata import entry_points

plugins = entry_points(group="vllm.general_plugins")
raise SystemExit(not any(item.name == "speculators_dfly" for item in plugins))
PY
then
    PLUGIN_METADATA_ROOT="$(mktemp -d /tmp/speculators-dfly-plugin.XXXXXX)"
    DIST_INFO="$PLUGIN_METADATA_ROOT/speculators_dfly_runtime-0.0.0.dist-info"
    mkdir -p "$DIST_INFO"
    printf '%s\n' \
        'Metadata-Version: 2.1' \
        'Name: speculators-dfly-runtime' \
        'Version: 0.0.0' > "$DIST_INFO/METADATA"
    printf '%s\n' \
        '[vllm.general_plugins]' \
        'speculators_dfly = speculators.vllm.dfly:register' \
...
echo "=== DFly evaluation configuration ==="
...
echo "=== Step 1: Launching vLLM DFly server ==="
setsid env \
    CUDA_VISIBLE_DEVICES="$CUDA_VISIBLE_DEVICES" \
    HF_ENDPOINT="$HF_ENDPOINT" \
    PYTHONPATH="$PLUGIN_PYTHONPATH${PYTHONPATH:+:$PYTHONPATH}" \
    VLLM_PLUGINS=speculators_dfly \
