#!/usr/bin/env bash
# Online DSpark Training Script (Qwen3.6-35B-A3B + perfectblend, full attention)
...
OUTPUT_DIR="${OUTPUT_DIR:-${RUN_DIR:-$WS/model_weights/dspark_qwen3_6-35b-a3b-perfectblend_2node}}"
...
RUN_NAME="${RUN_NAME:-dspark-${COMMENT:-base}-fullattn-2node-${JOB_TAG}}"
SAVE_DIR="${CHECKPOINT_DIR:-$OUTPUT_DIR/runs/$RUN_NAME}"
LOG_DIR="${LOG_DIR:-$REPO/logs/train/qwen3_6_35b_a3b/dspark_2node/$RUN_NAME}"
...
HS_PATH="$(mktemp -d "/tmp/dspark_node${NODE_RANK}.XXXXXX")"
VLLM_PID=""
TRAIN_PID=""
