#!/usr/bin/env bash
# Online DFlash2 Training Script (Qwen3.6-35B-A3B + open-perfectblend, full dataset)
...
OUTPUT_DIR="$WS/model_weights/dflash2_qwen3_6-35b-a3b-perfectblend"
...
HS_PATH="/tmp/hs_dflash2_qwen3_6_35b_a3b_${JOB_TAG}"
...
# DFlash2 requires the full verifier vocabulary.
for path in "$DATA_DIR/d2t.npy" "$DATA_DIR/t2d.npy"; do
    if [[ -e "$path" ]]; then
        echo "DFlash2 does not support pruned vocabulary: $path" >&2
        exit 1
    fi
done
...
RUN_NAME="dflash2-${COMMENT:-base}-$(date +%Y%m%d-%H%M%S)"
