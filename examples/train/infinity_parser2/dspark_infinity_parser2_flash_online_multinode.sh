#!/usr/bin/env bash
# 多机（SII 平台）在线 DSpark 训练：Infinity-Parser2.1-Flash，v2.1 语料。
# 配置与 dspark_infinity_parser2_flash_online.sh 保持一致，只加多机拓扑。
#
...
#
# 因为是 co-located，单节点内的 vLLM/训练卡数比、CPU 与 fetch 需求跟单机完全
# 相同，所以下面那些按 176 线程主机、6 个训练 rank 标定的参数逐节点原样沿用。
...
# 这套 RoCE fabric 是 rail-optimized 的：平台把 8 条 rail(mlx5_bond_1..8) 全塞进
# NCCL_IB_HCA，但不同 rail 的 NIC 之间并不互通，而且它们复用同一段地址 —— 所以跨
# rail 建 QP 时两端 GID 会撞成同一个，卡在 INIT->RTR 直到 ibv_modify_qp 报 110。
# co-located 只把尾部几张卡给训练，NCCL 建环时必然出现 GPU6->GPU7 这类跨 rail 的
# 跨机边（实测挂在 Channel 02 的环序 0 3 2 5 4 1 上），8 卡全训的作业反而碰不到。
# CROSS_NIC=0 要求一条通道两端用同一条 rail，跨 rail 的部分交给 PXN 走本机 NVLink
# 中转。仍连不通时依次试：NCCL_IB_HCA=mlx5_bond_8 把所有 rank 压到同一条 rail；
# 再不行 NCCL_IB_DISABLE=1 退回 bond0 上的 TCP（每步约 1.8 GB 梯度，带宽够用）。
...
echo "NCCL:          cross_nic=$NCCL_CROSS_NIC, ib_hca=${NCCL_IB_HCA:-<平台默认>}, ib_disable=${NCCL_IB_DISABLE:-0}"
echo "Model:         $MODEL"
echo "Data:          $DATA_DIR"
