...
- **Domino Training Algorithm**: Added support for the [Domino training algorithm](https://docs.vllm.ai/projects/speculators/en/latest/user_guide/algorithms/domino), which extends DFlash's anchored-block drafting with a GRU that carries state across the block, so each draft position conditions on the whole prefix before it rather than only the previous token. Its loss blends the corrected and uncorrected objectives with a weight that decays to zero over training, and existing Domino checkpoints convert with `convert_model(..., algorithm="domino")`.
